...
export const MusicPlayer: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 w-full h-dvh flex flex-col items-center justify-center bg-black overflow-hidden touch-none">
      ...
      >
        {/* Center: Art & Controls */}
        <div className="w-full lg:w-1/2 flex flex-col items-center justify-center gap-2 sm:gap-3 lg:gap-5 flex-1 lg:h-full min-h-0 overflow-hidden">
           {/* Album Art */}
        ...
        {/* Right: Playlist & Discovery — hidden on very small phones, scrollable on tablets */}
        <div 
          className="hidden sm:flex lg:w-1/2 h-[220px] sm:h-[280px] md:h-[320px] lg:h-full lg:max-h-[80vh] flex-col gap-3 rounded-xl sm:rounded-2xl lg:rounded-[2rem] p-3 sm:p-4 lg:p-6 backdrop-blur-3xl transition-colors duration-1000 bg-white/[0.03] border border-white/[0.04] overflow-hidden w-full"
          style={{ boxShadow: `0 20px 60px -10px rgba(0,0,0,0.5)` }}
        >
           <div className="flex items-center justify-between pb-2 sm:pb-3 border-b border-white/5 shrink-0">
              <h3 className="text-sm sm:text-base lg:text-lg font-bold tracking-tight text-white flex items-center gap-2 sm:gap-2.5">
                <span className="w-1 sm:w-1.5 h-4 sm:h-5 lg:h-6 rounded-full shadow-[0_0_8px_currentColor]" style={{ background: colors.primary, color: colors.primary }} />
                Up Next
              </h3>
              <div className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full bg-white/5 border border-white/5 text-[9px] sm:text-[10px] lg:text-xs font-bold text-white/40">
                {songs.length} Tracks
              </div>
           </div>
           ...
           <div className="pt-2 sm:pt-3 border-t border-white/5 shrink-0">
              <RelatedTracks />
           </div>
        </div>
      </motion.div>
